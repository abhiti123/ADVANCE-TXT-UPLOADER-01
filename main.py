# Don't Remove Credit Tg - @tg_abhi_bot
# Ask Doubt on telegram @tg_abhi_bot
import signal
import os
import re
import sys
...
🍁𝗠𝗲𝘀𝘀𝗮𝗴𝗲 𝗛𝗲𝗿𝗲 𝗧𝗼 𝗚𝗲𝘁 𝗔𝗰𝗰𝗲𝘀𝘀 @tg_abhi_bot
...
SUDO_USERS = [1096932670,2805832442,5070075289,5489195865,7216142187,5997896353,8175053386,7462419134]

AUTH_CHANNEL = 2805832442
...
async def upload(bot: Client, m: Message):
    ...
    credit = "️[🎭ʙᴀᴅꜰᴇʟʟᴀ🎭](https://t.me/tg_abhi_bot)"
    if raw_text3 == '1':
        CR = '[🎭ʙᴀᴅꜰᴇʟʟᴀ🎭](https://t.me/tg_abhi_bot)'
    elif raw_text3:
        try:
    ...
    try:
        for i in range(count - 1, len(links)):
            ...
            if "visionias" in url:
            ...
            elif "https://transcoded-videos-v2.classx.co.in/videos/vidyaeducationrahulsir-data/" in url:
                url = url.replace("https://transcoded-videos-v2.classx.co.in/videos/vidyaeducationrahulsir-data/", "")
                name1 = links[i][0].replace("\t", "").replace(":", "").replace("/", "").replace("+", "").replace("#", "").replace("|", "").replace("@", "@").replace("*", "").replace(".", "").replace("https", "").replace("http", "").strip()
                name = f'{str(count).zfill(3)}) {name1[:60]}'
                cmd = f'yt-dlp -o "{name}.mp4" "{url}"'
             ...
             url = f"https://anonymouspwplayer-b99f57957198.herokuapp.com/pw?url=%7Burl%7D?token=%7Bpw_token%7D"
            #elif '/master.mpd' in url:
             #id =  url.split("/")[-2]
